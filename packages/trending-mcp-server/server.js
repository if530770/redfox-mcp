...
const SERVER_NAME = 'redfox-mcp-server-trending';
...
const PLATFORM_LABEL = '热点研究';
const match = (n) => FILTER.prefixes.some(p => n.startsWith(p + '-')) || FILTER.includes.includes(n);

const { tools: allTools, skipped: allSkipped } = buildAllTools();
const tools = allTools.filter(t => match(t.name));
const skipped = allSkipped.filter(s => match(s.name));
...
console.error(`[${SERVER_NAME}] ${PLATFORM_LABEL} MCP Server v${SERVER_VERSION} 已启动`);
